...
digIds.forEach(val => {
 ...
 const classifications = [
  ...
  "Carson James", "Carson James", "Chad Henderson", "Christy Hamilton", "Cloud Diablo", "Cody Shindle",
  "Colton Amlin", "Cory Pelkman (D&L)", "Craig Erickson", "Craig Welsman", "Damon Keith", "Dan Therrien",
  "Daniel Pearo", "Danielle Nelson", "Darren Donley", "Darryl Meade (GPS)", "David Guild", "Derek Seward",
  ...
  "Jason Korotash", "Jason Stephenson", "Jeffery Harriman", "Jen Chubb", "Jeremy Fossum", "Jeremy Nellis",
  ...
  "Matthew Neilsen", "Melissa Medwid", "Michael Bruno", "Michael Martin", "Michael Mclelan", "Mike Cobban",
  ...
  "Theodore Uqhart", "Theressa Bartee", "Thomas Gregory", "Trent Morrison", "Trent Zevola", "Trevor Dewey",
  "Tristan Vigliotti", "Troy Connett", "Ty Stokes", "Tyler Brown", "Tyler Lothian", "William (Bill) Erskine",
...
function getFormData() {
  ...
  ["contractorSig", "clientSig"].forEach(id => {
    ...
    if (canvas) data[id] = canvas.toDataURL();
  });
  // Scope unit cell text content
  document.querySelectorAll("#scopeBody td[id^='scopeUnit']").forEach(td => {
...
function setFormData(data) {
  ...
  // Restore unit cell text
  for (const key in data) {
    if (key.startsWith("scopeUnit")) {
...
function addScopeRow() {
  ...
  document.getElementById("scopeBody").appendChild(row);
}
...
function addCrewRow() {
  ...
  document.getElementById("crewBody").appendChild(row);
}
...
window.addEventListener("load", () => {
  ...
  const maxScope = data ? getCachedRowCount("scopeId") : 5;
  const maxCrew = data ? getCachedRowCount("crewClass") : 5;
  const maxEquip = data ? getCachedRowCount("equipmentUnit") : 5;

  for (let i = 0; i < maxScope; i++) addScopeRow();
  for (let i = 0; i < maxCrew; i++) addCrewRow();
  for (let i = 0; i < maxEquip; i++) addEquipmentRow();

  if (data) requestAnimationFrame(() => setFormData(data));
});
